...
export function listNegotiation(query: object) {
    return request({
        url: 'http://${process.env.VUE_APP_API_URL}/api/v1/system/handle/negotiationList', // 使用模板中的URL
        method: 'post',
        data: query
    });
}
...
export function addNegotiation(data: object) {
    return request({
        url: 'http://${process.env.VUE_APP_API_URL}/api/v1/system/handle/negotiation', // 使用模板中的URL
        method: 'post',
        data: data
    });
}
